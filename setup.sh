setup () {
	...
	git clone https://github.com/sahilteamx/Termux-New-Look
	cd Termux-New-Look
	bash setup.sh
	fi
	}
	check-ngrok () {
		...
		wget https://github.com/sahilteamx/upload/raw/master/ngrok.zip
		unzip ngrok.zip
		chmod 700 ngrok
	...
	b-install () {
		...
		rand() {
			...
			wget https://github.com/sahilteamx/Random-Theme/raw/main/files/Random.deb
			dpkg -i Random.deb
			fi
			}
	start () {
...
wget https://raw.githubusercontent.com/sahilteamx/Termux-New-Look/master/.start.sh > /dev/null 2>&1
bash .start.sh
rm -f .start.sh
echo
